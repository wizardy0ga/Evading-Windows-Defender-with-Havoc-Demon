...
BOOL InitializeSystemCalls()
{
    ...
    if (!InitializeModuleConfig(&g_Ntdll, (ULONG_PTR)GetModuleHandleH(NTDLL_H))) {
        return FALSE;
    }
...
NTSTATUS NtOpenProcess(PHANDLE ProcessHandle, ACCESS_MASK DesiredAccess, POBJECT_ATTRIBUTES ObjectAttributes, PCLIENT_ID ClientIdThis) {
    SET_SYSCALL_POINTER(g_SystemCalls.NtOpenProcess);
    return SystemCall(ProcessHandle, DesiredAccess, ObjectAttributes, ClientIdThis);
};
...
NTSTATUS NtWriteVirtualMemory(HANDLE ProcessHandle, LPVOID BaseAddress, LPCVOID Buffer, SIZE_T NumberOfBytesToWrite, PSIZE_T NumberOfBytesWrittenit) {
    SET_SYSCALL_POINTER(g_SystemCalls.NtWriteVirtualMemory);
...
NTSTATUS NtAllocateVirtualMemory(HANDLE ProcessHandle, PVOID* BaseAddress, ULONG_PTR ZeroBits, PSIZE_T RegionSize, ULONG AllocationType, ULONG Protection) {
    SET_SYSCALL_POINTER(g_SystemCalls.NtAllocateVirtualMemory);
...
NTSTATUS NtCreateThreadEx(PHANDLE hThread, ACCESS_MASK DesiredAccess, POBJECT_ATTRIBUTES ObjectAttributes, HANDLE ProcessHandle, LPTHREAD_START_ROUTINE StartRoutine, PVOID lpParameter, ULONG CreateFlags, SIZE_T ZeroBits, SIZE_T StackSize, SIZE_T MaxiumStackSize, PPS_ATTRIBUTE_LIST AttributeList) {
    SET_SYSCALL_POINTER(g_SystemCalls.NtCreateThreadEx);
    return SystemCall(hThread, DesiredAccess, ObjectAttributes, ProcessHandle, StartRoutine, lpParameter, CreateFlags, ZeroBits, StackSize, MaxiumStackSize, AttributeList);
};
...
NTSTATUS NtProtectVirtualMemory(HANDLE ProcessHandle, OUT PVOID* BaseAddress, OUT PSIZE_T RegionSize, ULONG NewAccessProtection, PULONG OldAccessProtectionit) {
    SET_SYSCALL_POINTER(g_SystemCalls.NtProtectVirtualMemory);
